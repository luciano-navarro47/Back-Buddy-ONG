...
const getVeterinaryRandomData = async () => {
    ...
    const latitude = Number(faker_1.faker.number.float({ min: -35.10, max: -34.90, fractionDigits: 4 }));
    const longitude = Number(faker_1.faker.number.float({ min: -58.50, max: -58.30, fractionDigits: 4 }));
    const location = [latitude, longitude];
    const address = faker_1.faker.location.streetAddress();
    const email = faker_1.faker.internet.email({ firstName: name, provider: 'gmail.com' });
    let image = "";
    try {
